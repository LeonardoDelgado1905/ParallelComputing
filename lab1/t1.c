...
int main(void) {
    ...
    size_t img_size_2 = width * height * (channels+1);
    // Convert the input image to sepia
    unsigned char *sepia_img = malloc(img_size_2);
    if(sepia_img == NULL) {
    ...
    for(unsigned char *p = img, *pg = sepia_img; p != img + img_size; p += channels, pg += 4) {
        *pg       = (uint8_t)*p;
    ...
    stbi_write_png("./bicho_sepia.png", width, height, (channels+1), sepia_img, width * (channels+1));
